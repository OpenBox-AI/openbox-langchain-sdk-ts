    ...
    promises[operation] = async function patchedFsPromise(path: unknown, dataOrOptions?: unknown, maybeOptions?: unknown) {
      ...
      try {
        ...
        await evaluateFile(activityId, {
          filePath, fileMode, operation, stage: 'completed',
          startMs, endMs: Date.now(), bytesRead, bytesWritten,
        });
        return result;
      } catch (err) {
      ...
      if (originalCallback) {
        args[callbackIndex] = (...cbArgs: unknown[]) => {
          ...
          const bytesRead = !writes && (typeof data === 'string' || Buffer.isBuffer(data))
            ? Buffer.byteLength(data) : undefined;
          const bytesWritten = writes && (typeof args[0] === 'string' || Buffer.isBuffer(args[0]))
            ? Buffer.byteLength(args[0] as string | Buffer) : undefined;
          void evaluateFile(activityId, {
            filePath, fileMode, operation, stage: 'completed',
            startMs, endMs: Date.now(), error: err || undefined, bytesRead, bytesWritten,
          });
  ...
  try {
    ...
    for (const proto of prototypes) {
      ...
      proto.query = function patchedPgQuery(query: unknown, ...args: unknown[]) {
        ...
        const dbOpts = { dbSystem: 'postgresql' as const, dbName, statement, host, port: Number(port) || null };
        ...
        return evaluateDb(activityId, { ...dbOpts, stage: 'started', startMs })
          ...
          .then(() => (original.call(self, query, ...args) as Promise<{ rowCount?: number }>)
            .then(
              async (value) => {
                await evaluateDb(activityId, { ...dbOpts, stage: 'completed', startMs, endMs: Date.now(), rowcount: value?.rowCount }).catch(() => {});
                return value;
              },
              async (err) => {
                await evaluateDb(activityId, { ...dbOpts, stage: 'completed', startMs, endMs: Date.now(), error: err }).catch(() => {});
                throw err;
              },
            ),
          );
...
function patchMysql2Exports(mysql2: Record<string, unknown>): boolean {
  try {
    ...
    proto.query = function patchedMysqlQuery(sql: unknown, ...args: unknown[]) {
      ...
      const startMs = Date.now();
      const dbOpts = { dbSystem: 'mysql' as const, dbName: self.config?.database, statement, host: self.config?.host, port: Number(self.config?.port) || null };
      void evaluateDb(activityId, { ...dbOpts, stage: 'started', startMs });
      ...
      if (callbackIndex >= 0) {
        ...
        args[callbackIndex] = function patchedMysql2Callback(err: unknown, results: unknown, fields: unknown) {
          void evaluateDb(activityId, {
            ...dbOpts, stage: 'completed', startMs, endMs: Date.now(), error: err || undefined,
            rowcount: Array.isArray(results) ? results.length : (results as Record<string, unknown> | null)?.affectedRows as number ?? null,
          });
...
function patchDatabaseModuleLoader(): void {
  try {
    ...
    Module._load = function patchedModuleLoad(request: unknown, parent: unknown, isMain: unknown) {
      ...
      if (request === 'pg' && exported && typeof exported === 'object') patchPgExports(exported as Record<string, unknown>);
      else if (request === 'mysql2' && exported && typeof exported === 'object') patchMysql2Exports(exported as Record<string, unknown>);
      else if (request === 'mongodb' && exported && typeof exported === 'object') patchMongoExports(exported as Record<string, unknown>);
      else if (request === 'redis' && exported && typeof exported === 'object') patchRedisExports(exported as Record<string, unknown>);
      else if (request === 'ioredis' && exported && typeof exported === 'function') patchIoRedisExports(exported as { prototype?: Record<string, unknown> });
      return exported;
    };
...
function patchMongoExports(mongodb: Record<string, unknown>): boolean {
  ...
  for (const method of ['find', 'findOne', 'insertOne', 'updateOne', 'deleteOne', 'aggregate']) {
    ...
    proto[method] = function patchedMongoOperation(filter: unknown, ...args: unknown[]) {
      const self = this as { collectionName?: string; namespace?: string; dbName?: string; s?: { dbName?: string; namespace?: string } };
      const activityId = getCurrentActivityId();
      ...
      if (result && typeof result === 'object' && typeof (result as { then?: unknown }).then === 'function') {
        return (result as Promise<unknown>).then(
          async (value) => { await evaluateDb(activityId, { dbSystem: 'mongodb', dbName, statement, host: 'unknown', port: null, stage: 'completed', startMs, endMs: Date.now() }); return value; },
          async (err) => { await evaluateDb(activityId, { dbSystem: 'mongodb', dbName, statement, host: 'unknown', port: null, stage: 'completed', startMs, endMs: Date.now(), error: err }); throw err; },
        );
      }
      return result;
    };
...
function patchRedisClient(client: Record<string, unknown>): boolean {
  ...
  client.sendCommand = function patchedSendCommand(command: unknown, ...args: unknown[]) {
    ...
    const name = Array.isArray(command) ? String(command[0] ?? 'UNKNOWN') : String((command as Record<string, unknown> | null)?.name ?? command ?? 'UNKNOWN');
    ...
    if (result && typeof result === 'object' && typeof (result as { then?: unknown }).then === 'function') {
      return (result as Promise<unknown>).then(
        async (value) => { await evaluateDb(activityId, { dbSystem: 'redis', dbName: '0', statement, host: 'unknown', port: 6379, stage: 'completed', startMs, endMs: Date.now() }); return value; },
        async (err) => { await evaluateDb(activityId, { dbSystem: 'redis', dbName: '0', statement, host: 'unknown', port: 6379, stage: 'completed', startMs, endMs: Date.now(), error: err }); throw err; },
      );
    }
    return result;
  };
