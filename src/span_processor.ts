 ...
 * the relevant parts of WorkflowSpanProcessor.
 *
 * The Python SDK intercepts HTTP calls via OTel httpx instrumentation and patches
 * httpx.Client.send. In Node.js 18+ we patch globalThis.fetch the same way Python patches
 ...
 *   - Replaced IExecuteFunctions + openboxRequest with GovernanceConfig + axios.
 *   - registerActivity now takes GovernanceConfig + traceId instead of executeFunctions.
 */
...
async function evaluateHookSpan(
  ...
  const path = '/api/v1/governance/evaluate';
  try {
    ...
    const headers = buildSignedHeaders(
      ...
      bodyBytes,
      entry.config.apiKey,
      entry.config.agentDid,
      entry.config.agentPrivateKey,
    );
    if (entry.traceId) headers['X-OpenBox-Trace-Id'] = entry.traceId;
    const res = await axios.post<GovernanceVerdictResponse>(
      `${entry.config.openboxUrl}${path}`,
      bodyBytes,
      { headers, timeout: entry.config.governanceTimeout },
    );
    handleHookVerdict(res.data, String(spanData.http_url ?? spanData.name ?? 'hook'), entry.ctx.activity_id);
  } catch (err) {
...
function patchHttpModule(moduleName: 'node:http' | 'node:https'): boolean {
  try {
    ...
    const requestWrapper = function patchedRequest(this: unknown, ...args: unknown[]) {
        ...
        args[callbackIndex] = (response: {
          ...
          response.on?.('end', () => {
            ...
            void evaluateHookSpan(
              entry,
              buildHttpSpanData({ activityId, method, url, stage: 'completed', requestBody, responseBody, statusCode: response.statusCode ?? null, startMs, endMs }),
            );
          });
      ...
      void evaluateHookSpan(
        entry,
        buildHttpSpanData({ activityId, method, url, stage: 'started', requestBody: null, responseBody: null, statusCode: null, startMs }),
      ).catch((err) => {
      ...
      req.on?.('error', (err: unknown) => {
        ...
        void evaluateHookSpan(
          entry,
          {
            ...buildHttpSpanData({ activityId, method, url, stage: 'completed', requestBody: chunksToText(reqBodyChunks), responseBody: null, statusCode: null, startMs, endMs }),
            error: String(err),
...
export function registerActivity(
  ...
  patchFetch();
  _activeActivities.set(activityId, { ctx, config, traceId });
}
